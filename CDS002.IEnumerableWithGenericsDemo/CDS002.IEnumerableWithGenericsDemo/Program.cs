        ...
        public static void TestStreamReaderEnumerable(string keyString)
        {
            ...
            try
            {
                ...
                foreach (var item in stringsFound)
                {
                    ...
                    {
                        int index = item.IndexOf(keyString,location);
                        int cutout = item.Length - index;
                        if (location< item.Count() && index != -1)
                        {
                            if (cutout < 13)
                            {
                                string x = item.Substring(index, cutout);
                                Console.WriteLine("{0}.第{1}行,第{2}个字母开始:“{3}”；", count, count, index + 1, x);
                                location = index + cutout;
                            }
                            else
                            {
